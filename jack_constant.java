...
public class jack_constant extends GhidraScript {
	...
	public String getRowObjectValue(AddressableRowObject rowObject) {
...
/*
				...
				return "";
*/
			return getRowObjectValue(rowObject);
			}
		};
	...
	private void addConstants(TableChooserDialog tableChooserDialog,
		...
		for (int i = 0; i < keyArray.length; i++) {
			...
			ConstUseLocation useLoc = new ConstUseLocation(currentProgram, loc, constant, null);
			decideRenameFunc(useLoc);
			tableChooserDialog.add(useLoc);
		}
	}

  private void decideRenameFunc(ConstUseLocation useLoc) {
		Function f = useLoc.getProgram().getFunctionManager().getFunctionContaining(useLoc.getAddress());
		if (f == null) {
			println("func was null?");
			return;
		}
		Long val = useLoc.getConstValue();
		println(String.format("decideRename: %s to %s", f.getName(), getRowObjectValue(useLoc)));
	} 
